function low_level_feature=getLowLevelFeature(img_path)
...
horizon_length=size(img_ycbcr,1)/7;
mean_y=zeros(1,7);
mean_b=zeros(1,7);
mean_r=zeros(1,7);
sdev_y=zeros(1,7);
...
for i=1:7
    if(i<7)
        temp_y=img_ycbcr((i-1)*horizon_length+1:i*horizon_length,:,1);
        temp_b=img_ycbcr((i-1)*horizon_length+1:i*horizon_length,:,2);
        temp_r=img_ycbcr((i-1)*horizon_length+1:i*horizon_length,:,3);
    else
        temp_y=img_ycbcr((i-1)*horizon_length+1:end,:,1);
        temp_b=img_ycbcr((i-1)*horizon_length+1:end,:,2);
        temp_r=img_ycbcr((i-1)*horizon_length+1:end,:,3);
    end
